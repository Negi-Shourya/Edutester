#!/usr/bin/env node
/**
 * Read-only: dump NEET 2023 Physics questions (numbers 1-50) with
 * full stem + options from Supabase, flagging blank/missing text.
 *
 * Run:  node scripts/_dump-neet-2023-physics.mjs
 */
...
const physics = qs.filter((q) => q.number >= 1 && q.number <= 50);
const ids = physics.map((q) => q.id);
...
for (const q of physics) {
  ...
  const truncated = !q.text || q.text.trim().length < 40 || /[|]\s*$/.test(q.text);
...
console.log(`\nFlagged questions: ${flagCount} / ${physics.length}`);
process.exit(0);
