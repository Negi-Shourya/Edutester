...
function PaperEntryRow({
  ...
  return (
    <div>
      ...
      {/* Hard-locked papers get a full-width notice rather than a paywall — there
          is nothing to buy, the paper simply is not ready yet. */}
      {isHardLocked && (
        <div className="mx-4 mb-3 flex items-center gap-2 px-3 py-2 bg-yellow-50 border border-yellow-200 rounded-lg text-xs text-yellow-800">
...
interface SessionGroup {
  ...
  badge: string;
  papers: PaperEntry[];
  /** Placeholder section — no papers seeded yet, renders as a "Coming Soon" card. */
  comingSoon?: boolean;
}
...
export default function PaperTests() {
  ...
  const sessions: SessionGroup[] = [
    ...
    },
    {
      ...
      accent: 'from-stone-50 to-stone-100',
      badge: 'Coming Soon',
      papers: [],
      comingSoon: true,
    },
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {exam === 'neet' ? (
          ...
          <div className="space-y-6">
            {sessions.map((session) => {
              // Placeholder years have nothing to expand — render a flat,
              // non-interactive card instead of an accordion that opens onto air.
              if (session.comingSoon) {
                return (
                  <div
                    key={session.id}
                    className="bg-white/60 rounded-2xl border border-dashed border-stone-200 overflow-hidden"
                  >
                    <div className="flex items-center justify-between px-6 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-left">
                          <h2 className="text-base font-medium text-stone-500">{session.label}</h2>
                          <p className="text-xs text-stone-400">{session.subtitle}</p>
                        </div>
                      </div>
                      <span className="text-xs font-medium text-stone-400 bg-stone-50 border border-stone-200 px-3 py-1 rounded-full">
                        Coming Soon
                      </span>
                    </div>
        ...
        {/* `animate-in fade-in zoom-in` used to be here, but tailwindcss-animate
            is not a dependency — the classes were inert. motion/react is. */}
        <AnimatePresence>
          {showLockedToast && (
            <motion.div
              className="fixed top-6 left-1/2 z-50 bg-yellow-50 text-yellow-800 px-5 py-3 rounded-lg shadow-2xl flex items-center gap-2.5 text-sm font-medium border border-yellow-200"
