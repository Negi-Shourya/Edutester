#!/usr/bin/env node
/**
 * Seeds NEET 2024 extracted questions into the Supabase question bank.
 *
 * Reads neet-out/2024/questions.json + images, uploads images to storage,
 * inserts paper/sections/questions/options into the database.
 *
 * Run:  node scripts/seed-neet-2024.mjs [--force]
 */
...
const OUT_DIR = join(__dirname, "..", "neet-out", "2024");
  ...
  try {
    ...
    do {
      const { data: lst, error: listErr } = await supabase.storage
        .from(BUCKET)
        .list("neet-2024", { limit: 200, offset: 0 });
      if (listErr) throw listErr;
      if (!lst || lst.length === 0) break;
      const names = lst.map((o) => `neet-2024/${o.name}`);
...
async function seed() {
  ...
  // No subsections for this paper (NEET 2024 doesn't use Section A/B split)
  ...
  for (const [idx, q] of data.questions.entries()) {
    ...
    for (const opt of q.options ?? []) {
      optionRows.push({
        ...
        text: opt.text ?? "",
        figure_url: opt.figure || opt.image ? `${PUB_BASE}/neet-2024/${opt.figure ?? opt.image}` : null,
      });
    }
