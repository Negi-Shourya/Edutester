...
const { data: qs } = await sb.from('questions').select('id').eq('paper_id', 1);
const { data: keys } = await sb.from('question_keys').select('question_id,correct_answer,solution').in('question_id', qs.map(q=>q.id));
const bySec = {};
for (const q of qs) bySec[q.id] = q;
let numbered = 0, badEnd = 0; const lens = { math: [], phy: [], chem: [] };
const { data: secs } = await sb.from('sections').select('id,name').eq('paper_id', 1);
const secName = Object.fromEntries(secs.map(s=>[s.id, s.name]));
const { data: qsec } = await sb.from('questions').select('id,section_id').eq('paper_id', 1);
