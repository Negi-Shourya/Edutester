...
const paperKey = '22-jan-morning';
const pid = (await sb.from('papers').select('id').eq('key', paperKey).single()).data.id;
const { data: qs } = await sb.from('questions').select('id').eq('paper_id', pid);
const { data: keys } = await sb.from('question_keys').select('question_id,correct_answer,solution').in('question_id', qs.map(q=>q.id));
const { data: secs } = await sb.from('sections').select('id,name').eq('paper_id', pid);
const secName = Object.fromEntries(secs.map(s=>[s.id, s.name]));
const { data: qsec } = await sb.from('questions').select('id,section_id').eq('paper_id', pid);
const qm = Object.fromEntries(qsec.map(q=>[q.id, secName[q.section_id].slice(0,4)]));
let numbered = 0, badEnd = 0; const lens = { math: [], phy: [], chem: [] };
for (const k of keys) {
  const s = k.solution || '';
  const ls = s.split('\n').filter(l=>l.trim());
  const g = qm[k.question_id] === 'Math' ? 'math' : (qm[k.question_id] === 'Phys' ? 'phy' : 'chem');
  lens[g].push(ls.length);
