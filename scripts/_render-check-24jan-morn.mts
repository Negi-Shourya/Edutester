...
const sols = JSON.parse(readFileSync('./scripts/solutions-24-jan-morning.json','utf8'));
...
for (const [qid, sol] of Object.entries(sols)) {
  ...
  // formatting guards
  if (/(^|\n)\s*(\d+[.)]\s|Step\s*\d+|[•\-*]\s)/i.test(sol) && !/\\times/.test(sol.slice(0,10))) {
    console.log(`FORMAT WARN ${qid}: possible numbering`);
  }
