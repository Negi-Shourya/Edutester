#!/usr/bin/env node
/**
 * Seeds Re-NEET (UG) 2026 into the Supabase question bank.
 *
 * Reads neet-out/reneet-2026/questions.json + images (built by
 * scripts/_assemble-reneet-2026.mjs), uploads the figures to storage, and
 ...
 *
 * Run:  node scripts/seed-reneet-2026.mjs [--force]
 */
...
const FOLDER = "reneet-2026";
const YEAR = 2026;

const __dirname = dirname(fileURLToPath(import.meta.url));
const OUT_DIR = join(__dirname, "..", "neet-out", "reneet-2026");
...
async function seed() {
  ...
  if (!existsSync(file)) {
    console.error(`Missing ${file} — run scripts/_assemble-reneet-2026.mjs first`);
    process.exit(1);
  }
